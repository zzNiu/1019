    ...
    def calculate_station_module_requirements(self, n: int, k: int,
        ...
        return {
            ...
            'freight_analysis': {
                ...
                'total_demand': total_demand_need,
...
# ====================================================================================================================
# 【方案一】修改后的核心仿真函数：模拟并**生成**模块调整策略，用于创建初始个体
# ====================================================================================================================
def simulate_with_integrated_module_system(individual, parameters, global_demand_data,
    ...
    # 此函数与原始版本逻辑一致，用于生成和完善个体
    # ... (此部分代码保持不变，因为它用于随机生成模块调整策略)
    # 唯一的变化是为了避免混淆，在gen_individual中调用此函数时，不再返回module_adjustments，而是将其附加到individual中。
    # 为了实现干净的拆分，我将原函数重命名为 simulate_and_generate_adjustments
    # 但根据用户要求，为了方便理解，这里我们保留原函数名，只在注释中说明其用途。
    """
    集成化仿真函数 - 结合递推关系和仿真验证
    注意：此函数用于在创建新个体时，**生成**其模块调整策略
    """
    # 确保个体结构完整，但module_adjustments和adjustment_ranges可以为空
    # ... (与原函数保持一致)
    ...
    # 用于存储生成的模块调整策略和范围
    generated_module_adjustments = {"up": {}, "down": {}}
    generated_adjustment_ranges = {"up": {}, "down": {}}
    ...
    for direction in ["up", "down"]:
        ...
        for vehicle in all_vehicles:
            ...
            for sid in range(vehicle["num_stations"]):
                ...
                # 计算下车数量
                alighted_p = sum(
                    sum(p_dict.values()) for dest, p_dict in onboard_passengers.items() if dest == station_id)
                ...
                # 2. 执行模块调整（在上车前）
                # 【关键修改】这里使用原有的随机生成逻辑，用于创建初始个体
                adjusted_p_modules, adjusted_f_modules, delta_p, delta_f = module_system.generate_feasible_module_allocation(
                    module_analysis)
                ...
                if sid < vehicle["num_stations"] - 1:  # 不是最后一站
                    ...
                    # 【新增】记录生成的模块调整策略和范围
                    if vid not in generated_module_adjustments[direction]:
                        generated_module_adjustments[direction][vid] = {}
                        generated_adjustment_ranges[direction][vid] = {}

                    generated_module_adjustments[direction][vid][station_id] = {
                        "delta_p": delta_p,
                        "delta_f": delta_f
                    }

                    generated_adjustment_ranges[direction][vid][station_id] = {
    ...
    return vehicle_schedule, total_cost, remaining_passengers, remaining_freights, failure_records, df_enriched, module_analysis_records, cost_components, generated_module_adjustments, generated_adjustment_ranges
