...
def generate_comprehensive_cost_evolution_plot(logbook, save_dir):
    ...
    if hasattr(logbook, '__iter__') and hasattr(logbook[0], 'get'):
        ...
        min_costs = [record.get('min', float('inf')) for record in logbook if
                     np.isfinite(record.get('min', float('inf')))]
        avg_costs = [record.get('avg', float('inf')) for record in logbook if
                     np.isfinite(record.get('avg', float('inf')))]
    else:
        # 如果是DEAP logbook格式
    ...
    # 生成综合分析图表
    _generate_comprehensive_analysis_plot(generations, min_costs, avg_costs, save_dir)

    print(f"✅ 所有成本进化曲线图表已保存到: {save_dir}")


# 12个子图
def _generate_comprehensive_analysis_plot(generations, min_costs, avg_costs, save_dir):
    """生成综合分析图表"""
    ...
    best_single_improvement = max([min_costs[i - 1] - min_costs[i] for i in range(1, len(min_costs))]) if len(
        min_costs) > 1 else 0
